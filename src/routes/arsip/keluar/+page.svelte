<script lang="ts">
	import { Search, Filter, Plus, Send, ChevronRight, LayoutGrid, List, FileBadge } from '@lucide/svelte';
...
<div class="space-y-4">
			...
			{#each arsipKeluar as surat}
				<div class="rounded-xl border border-slate-100 bg-white p-4 shadow-sm active:bg-slate-50 relative">
					{#if surat.file_url}
						...
						<div class="absolute right-4 top-4 text-primary-600" title="Dokumen digital tersedia">
							<FileBadge size={20} />
						</div>
					{/if}
					...
					<div class="flex items-start justify-between gap-2">
						<div class="flex-1 space-y-2">
							<div class="flex items-center gap-2 flex-wrap pr-8">
								...
								<span class="text-xs font-medium text-slate-500">{surat.no_index}</span>
								<span class="text-xs text-slate-500 ml-auto">{formatDate(surat.tanggal_pembuatan)}</span>
							</div>
							<h3 class="font-medium text-slate-800 leading-tight">{surat.perihal || '-'}</h3>
							<div class="space-y-1">
								<p class="text-sm text-slate-600 flex items-center gap-1">
									<Send size={14} class="text-slate-400 flex-shrink-0"/>
									<span class="truncate">{surat.tujuan || '-'}</span>
								</p>
			...
			<table class="w-full text-left text-sm text-slate-600">
				<thead class="bg-slate-50 text-xs uppercase text-slate-500 border-b border-slate-200">
					<tr>
						...
						<th class="px-4 py-3 font-semibold whitespace-nowrap">No. Index</th>
						<th class="px-4 py-3 font-semibold whitespace-nowrap">Tgl. Pembuatan</th>
						<th class="px-4 py-3 font-semibold min-w-[200px]">Tujuan</th>
				...
				<tbody class="divide-y divide-slate-100">
					...
					{#each arsipKeluar as surat}
						<tr class="hover:bg-slate-50">
							<td class="px-4 py-3">
								{#if surat.file_url}
									<a href={surat.file_url} target="_blank" class="text-primary-600 hover:text-primary-700" title="Lihat Dokumen">
										<FileBadge size={18} />
									</a>
								{:else}
							...
							<td class="px-4 py-3">{surat.no_index || '-'}</td>
							<td class="px-4 py-3 whitespace-nowrap">{formatDate(surat.tanggal_pembuatan)}</td>
	...
	<!-- FAB -->
	<a href="/arsip/keluar/tambah" class="fixed bottom-20 right-4 flex h-14 w-14 items-center justify-center rounded-full bg-primary-600 text-white shadow-lg shadow-primary-600/30 transition-transform active:scale-95 hover:bg-primary-700">
		<Plus size={28} />
	</a>
</div>
