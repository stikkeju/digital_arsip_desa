<script lang="ts">
	import { Search, Filter, Plus, FileText, LayoutGrid, List, FileBadge, ExternalLink, Download, FileType2, XCircle } from '@lucide/svelte';
	...
	function clearDashboardFilter() {
		searchQuery = '';
		goto('/arsip/masuk', { replaceState: true });
	}
	...
	function handleExport() {
		const exportData = filteredArsip.map((s, i) => ({
			...
			'Nomor Surat Asli': s.no_surat,
			'Tanggal Terima': formatDate(s.tanggal_terima),
			'Pengirim': s.pengirim,
			'Perihal': s.perihal,
			'Keterangan': s.keterangan || ''
		}));
		const title = `Surat_Masuk_${filterMonth || 'SemuaBulan'}_${filterYear || 'SemuaTahun'}`;
	...
	let filteredArsip = $derived(
		arsipMasuk.filter((surat) => {
			...
			if (urlTimeFilter && urlTimeFilter !== 'all') {
				if (!isDateInRange(surat.tanggal_terima, urlTimeFilter)) return false;
...
<div class="flex h-full flex-col">
			...
			{#if viewMode === 'card'}
				...
				<div class="space-y-4 pb-4">
					{#each filteredArsip as surat}
						...
						<div 
							onclick={() => goto(`/arsip/masuk/${surat.id}`)}
							class="group overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition-all hover:shadow-md hover:border-blue-300 cursor-pointer"
						>
							<!-- Card Header -->
							...
							<div class="p-4 space-y-3 relative">
								<h3 class="text-[15px] font-bold text-slate-800 leading-snug pr-8 group-hover:text-blue-700 transition-colors">{surat.perihal || 'Tanpa Perihal'}</h3>
								...
								{#if surat.file_url}
									<div class="pt-2">
										<a 
											...
											class="inline-flex items-center gap-1.5 text-xs font-semibold text-blue-600 hover:text-blue-700 bg-blue-50 px-3 py-1.5 rounded-lg border border-blue-100 transition-colors"
										>
											<FileBadge size={14} />
											Buka Dokumen
									...
									<td class="px-4 py-3 text-center align-middle">
										{#if surat.file_url}
											<a 
												...
												target="_blank" 
												onclick={(e) => e.stopPropagation()}
												class="inline-flex items-center justify-center p-1.5 text-blue-600 bg-blue-50 rounded-lg hover:bg-blue-100 transition-colors" 
												title="Lihat Dokumen"
											>
												<FileBadge size={18} />
											</a>
	...
	{#if auth.isOperator}
	<button 
		class="fixed bottom-24 right-6 flex h-14 w-14 items-center justify-center rounded-full bg-blue-600 text-white shadow-lg shadow-blue-600/30 transition-transform hover:scale-105 active:scale-95 z-[60]"
		onclick={() => goto('/arsip/masuk/tambah')}
	>
